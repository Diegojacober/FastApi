...
#By pass warning SQLModel select
...
async def get_curso(curso_id: int = Path(title="ID do curso desejado", description="Deve ser maior que 0", gt=0), db: AsyncSession = Depends(get_session)):
    async with db as session:
...
async def delete_curso(id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if curso_del:
            ...
            # Necessário por cause de um bug na fastapi
